...
from model import SFFCConfig, ScoreFusion, FusionConcat, SoftMatchWeighting, FreeMatchThresholding
import time
        ...
        for (f_batch, s_batch, y_batch), (f_batch_unl, s_batch_unl) in zip(
            ...
            with autocast():
                ...
                if use_ssl:
                    ...
                    target_dist = freematch.fairness_target()  # still no-grad, that's correct — it's the target, not the prediction
                    fairness_loss = F.kl_div(pred_dist.clamp(min=1e-8).log(), target_dist, reduction="sum")
                
                    loss = sup_loss + lambda_u * unsup_loss + lambda_f * fairness_loss
                else:
                    loss = sup_loss
