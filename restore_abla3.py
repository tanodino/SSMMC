...
# ==========================================================================
# Restores OURS_ABLA3_NoMLA_NOSSL checkpoints (ablation: no multi-layer
# fusion AND no continual SSL during fine-tuning -- classification uses
# each encoder's FINAL-layer CLS token directly, and training loss was
# `loss = loss_cls` only).
#
# Architecturally identical to PretrainModelV4_NoALF from ablation 2 --
# the only training-time difference (loss = loss_cls vs. the full SSL +
# classification loss) doesn't change what's stored in the checkpoint,
# same as the ablation 1 vs. original case. So this is ablation 2's
# restore script with the output directory updated.
# ==========================================================================
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Restore OURS_ABLA3_NoMLA_NOSSL (no multi-layer fusion, no continual SSL) "
                    "checkpoints and evaluate weighted F1 on the test split."
    )
    ...
    parser.add_argument("--output_dir", type=str, default="OURS_ABLA3_NoMLA_NOSSL",
                        help="output directory, default OURS_ABLA3_NoMLA_NOSSL")
