...
class LightweightLayerFusion(nn.Module):
    """A learned SCALAR weight per layer (no Q/K/V projections), softmax-
    normalized, then a weighted sum of the per-layer CLS tokens.
    Parameter count = num_layers (e.g. 4) -- negligible overfitting risk
    on a ~50-sample labeled set, unlike full cross-attention (see module
    docstring)."""
    ...
    def forward(self, layer_tokens: torch.Tensor) -> torch.Tensor:
        # layer_tokens: [B, L, D]
        weights = F.softmax(self.layer_logits, dim=0)          # [L]
...
class PretrainModel(nn.Module):
    ...
    def __init__(self, config: SFFCConfig, num_classes: int, layer_indices: list, embed_dim: int = 384):
        ...
        self.alf_m1 = LightweightLayerFusion(num_layers=len(layer_indices))
        self.alf_m2 = LightweightLayerFusion(num_layers=len(layer_indices))
...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % 5 == 0:
            ...
            layer_weights_m1 = F.softmax(model.alf_m1.layer_logits, dim=0).detach().cpu().numpy().round(3)
            layer_weights_m2 = F.softmax(model.alf_m2.layer_logits, dim=0).detach().cpu().numpy().round(3)
